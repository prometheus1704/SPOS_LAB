...
public class SPOS3_PRIORITY {
    ...
    public static void showCompleteChart() {
        ...
        System.out.printf("%-10s%-10s%-10s%-10s%-10s%-10s%-12s%-10s%n",
                "Process", "Arrival", "CPU", "Priority", "Start", "Finish",
                "Turnaround", "Waiting");
    ...
    static public void PRIORITY() {
        ...
        for (int i = 1; i < processes.size(); i++) {
            ...
            if (processArrival > end) {
                ...
                ganttChart.add(new process(p.processName, start, end, p.priority));
            } else {
                start = end;
                end += processCPU;
                ganttChart.add(new process(p.processName, start, end, p.priority));
    ...
    public static void main(String[] args) {
        processes.add(new process("P1", 0, 3, 5));
        processes.add(new process("P2", 4, 1, 4));
        processes.add(new process("P3", 3, 2, 3));
        processes.add(new process("P4", 1, 5, 2));
        processes.add(new process("P5", 12, 5, 1));
...
class process {
    ...
    process(String processName, int arrivalTime, int CPUTime, int priority) {
