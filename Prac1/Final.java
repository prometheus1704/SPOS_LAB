...
class AssemblerTables {
    public static void main(String[] args) throws Exception {
        List<String> code = readFile("prac1.txt");
    ...
    private static List<String> generateIntermediateCode(List<String> code,
        ...
        for (String line : code) {
            ...
            if (parts[0].equals("START") || parts[0].equals("END") || parts[0].equals("LTORG")) {
                // Handle directives
                codeLine.append("(").append(motTable.get(parts[0]).type).append(", ")
                        .append(motTable.get(parts[0]).code).append(") ");
                intermediateCode.add(codeLine.toString().trim());
                continue;
            }
            
            if (parts[0].equals("ORIGIN") || parts.length > 1 && parts[1].equals("EQU")) {
                continue;
            }
            ...
            for (int i = 0; i < parts.length; i++) {
                if (motTable.containsKey(parts[i])) {
                    codeLine.append("(").append(motTable.get(parts[i]).type).append(", ")
                            .append(motTable.get(parts[i]).code).append(") ");
                } else if (symbolTable.containsKey(parts[i])) {
                    codeLine.append("(S, ").append(symbolTable.get(parts[i]).address).append(") ");
                } else {
                    ...
                    if (!literalFound) {
                        codeLine.append(parts[i]).append(" ");
                    }
                }
            }
            intermediateCode.add(codeLine.toString().trim());
...
class MOTEntry {
    ...
    public MOTEntry(String name, String type, String code) {
        this.name = name;
        this.type = type;
        this.code = code;
    }
