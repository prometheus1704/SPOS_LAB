...
public class SPOS3_RoundRobin {
    public static List<process> processes = new ArrayList<>();
    public static List<process> ganttChart = new ArrayList<>();
    public static int fixedCPUTime = 2;
    public static int start;
    public static int end;
    public static int time;

    public static void showCompletedTask(process p){
        System.out.println(String.format("Task Executed: %-5s Arrival Time: %-5d Current CPU time: %-3d", p.processName, p.arrivalTime, p.CPUTime));
    }
    ...
    // i have absolutely no idea of this is correct or not
    ...
    public static void roundRobin(){
        ...
        while(!q.isEmpty()){
            ...
            if(p.CPUTime <= fixedCPUTime){
                if(p.CPUTime < time-start){
                    ganttChart.add(new process(p.processName, start, start+p.CPUTime));
                }
                else{
                    ganttChart.add(new process(p.processName, start, time));
                }
                p.CPUTime = 0;
                start = time;
            }else{
                p.CPUTime -= fixedCPUTime;
                q.offerLast(p);
            }
            showCompletedTask(p);
        }
    ...
    public static void main(String[] args) {
        ...
        // i wrote this here cause if we write it at the time or declaration only then
        // we may get out of bound exception
        start = processes.get(0).arrivalTime;
        time = start;
...
// cause java dosent support struct :(
class process{
    String processName;
    int arrivalTime;
    int CPUTime;
